        ...
        <div class="col-md-8">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <?php endif; ?>
                    <form action="<?= BASE_URL_ADMIN ?>&action=edit-movie&movie_id=<?= $movie['movie_id'] ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="price" name="price" value="<?= $movie['price'] ?>" min="0">
                        </div>
                        <div class="mb-3">
                            <label for="description" class="form-label">Mô Tả</label>
                            <textarea class="form-control" id="description" name="description" rows="4"><?= htmlspecialchars($movie['description']) ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="banner" class="form-label">Hình Ảnh (Banner)</label>
                            <input type="file" class="form-control" id="banner" name="banner" accept="image/*">
                            <small>Hiện tại: <img src="<?= PATH_ASSETS_UPLOAD . $movie['banner'] ?>" alt="Banner" width="100"></small>
                        </div>
                        <div class="mb-3">
                            <label for="video_url" class="form-label">Tải Lên Video</label>
                            <input type="file" class="form-control" id="video_url" name="video_url" accept="video/*">
                            <small>Hiện tại: <?= $movie['video_url'] ? '<a href="' . PATH_ASSETS_UPLOAD . $movie['video_url'] . '" target="_blank">Xem video</a>' : 'Không có' ?></small>
                        </div>
                        <div class="mb-3">
                            <label for="download_url" class="form-label">Tải Lên Tệp</label>
                            <input type="file" class="form-control" id="download_url" name="download_url" accept=".zip,.rar,.7z,.pdf,.mp4">
                            <small>Hiện tại: <?= $movie['download_url'] ? '<a href="' . PATH_ASSETS_UPLOAD . $movie['download_url'] . '" target="_blank">Tải tệp</a>' : 'Không có' ?></small>
                        </div>
                        <div class="d-grid">
